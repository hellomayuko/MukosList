        ...
        NavigationView {
            VStack {
                ...
                List(shoppingLists, id: \.self) { shoppingList in
                    NavigationLink(destination: ShoppingItemsView(isBeingPresented: self.$presentAddItemView, isPresentedFromAddListView:.constant(false), navigationBarHidden: self.$navigationBarHidden, listName: shoppingList.name ?? "idk").environment(\.managedObjectContext, self.managedObjectContext)) {
